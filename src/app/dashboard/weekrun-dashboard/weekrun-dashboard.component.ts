...
import { RunService, RunWeek } from '../../run/run.service';
...
@Component({
  selector: 'app-weekrun-dashboard',
  standalone: true,
  providers: [RunService],
  imports: [NgApexchartsModule, HttpClientModule],
  templateUrl: './weekrun-dashboard.component.html',
  styleUrl: './weekrun-dashboard.component.scss'
})
export class WeekrunDashboardComponent {
  ...
  ngOnInit(): void {
    this.runService.getRunWeekTotal().subscribe(data => {
      this.weekRuns = data;
      this.initChartData();
    });
  }
  ...
  public initChartData(): void {
    ...
    this.weekRuns.sort((a, b) => parseInt(b.week) - parseInt(a.week));

    let lastWeek = this.weekRuns[0].week;
    let firstWeek = this.weekRuns[this.weekRuns.length - 1].week;
  ...
  getYearWeeksBetween(firstWeek: string, lastWeek: string) {
    ...
    while (currentYear < endYear || (currentYear === endYear && currentWeek <= endWeek)) {
      ...
      // Remplir yearWeeks
      let weekExist = false;
      for (let i = 0; i < this.weekRuns.length; i++) {
        if (this.weekRuns[i].week == yearWeek) {
          weekExist = true;
          this.yearWeeks[indexWeek] = { week: this.getFirstAndLastDayFromYearWeek(yearWeek), yearweek: parseInt(yearWeek), distSum: this.weekRuns[i].distSum, timeSum: this.weekRuns[i].timeSum, denivSum: this.weekRuns[i].denivSum };
        }
      }
      if (weekExist == false) {
